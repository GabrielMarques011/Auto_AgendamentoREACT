  ...
  export default function ScreenConexao2({ formData, setFormData, nextStep, prevStep }) {
    ...
    // CORREÇÃO: Atualiza a observação quando o checklist muda
    useEffect(() => {
      const checklistText = buildChecklistText(checklistItems);
      
      // Remove qualquer texto anterior de "Processos feitos:" da observação
      const baseText = formData.observacao?.replace(/Processos feitos:.*?\./g, '').trim() || '';
      ...
      if (newObservacao.trim() !== (formData.observacao || '').trim()) {
        setFormData({ 
          ...formData, 
    ...
    const handleObservacaoChange = (e) => {
      ...
      // Atualiza o checklist baseado no texto (opcional - remove se não quiser essa funcionalidade)
