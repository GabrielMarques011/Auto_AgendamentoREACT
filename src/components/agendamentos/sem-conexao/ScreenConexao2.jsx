...
export default function ScreenMudanca2({ formData, setFormData, nextStep, prevStep }) {
  ...
  useEffect(() => {
    console.log("🔍 ScreenMudanca2 - Props recebidas:", {
  ...
  useEffect(() => {
    ...
    if (!checklistText) {
      const cleanText = formData.observacao?.replace(/Foi realizado:.*?\./g, '').trim() || '';
      if (cleanText !== formData.observacao) {
        setFormData({ 
          ...formData, 
          observacao: cleanText
        });
      }
      return;
    }

    const hasChecklist = formData.observacao?.includes("Foi realizado:");
    
    let newObservacao;
    if (hasChecklist) {
      newObservacao = formData.observacao.replace(/Foi realizado:.*?\./g, checklistText);
    } else {
      const baseText = formData.observacao?.trim() || '';
      newObservacao = baseText ? `${baseText} ${checklistText}` : checklistText;
    }
  ...
  const handleObservacaoChange = (e) => {
    ...
    updateChecklistFromText(newValue);
  };

  const updateChecklistFromText = (text) => {
    const updatedItems = checklistItems.map(item => ({
      ...item,
      checked: text.includes(item.text)
    }));
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="space-y-6 flex-1">
        ...
        <div className="flex justify-between pt-6">
          <button 
            onClick={handleVoltar} // ← Agora usa handleVoltar
